...
import { UsersController, MainController, NewsController, AuthController } from './controllers'
...
const CORS_OPTIONS = {
  origin: 'https://hw15-server.vercel.app',
...
class App {
  ...
  private routing (): void {
    ...
    this.app.post('/api/newsposts', passport.authenticate('bearer', { session: false }), createUpdateMiddlewares, this.newsController.create)
    ...
    this.app.put('/api/newsposts/:id', createUpdateMiddlewares, this.newsController.update)
	 ...
	 this.app.get('/api/user', passport.authenticate('bearer', { session: false }))

	 this.app.get('/api/newsposts/error', (_req: Request, _res: Response, next: NextFunction) => {
      try {
